...
!3       --> Transbnd. matrix-based  4      --> Muskingum-Cunge
!5       --> Muskingum-Cunge (needing HAND geometry)
PetscInt :: IS_opt_run
!1       --> regular run             2      --> parameter optimization
PetscInt :: IS_opt_phi
!1       --> phi1                    2      --> phi2
PetscInt :: IS_opt_ov
!0       --> no delay   1       --> consider overland delay process
REAL :: beta
!adjustment factor to determine how Tov varies with Qlat (default 0.5)
PetscInt :: Is_opt_weighted
!0       --> no weight (centroid-based method)
!1       --> weighted (needing ArcGIS toolbox to extract weighted table)
character(len=120) :: rapid_weighted_file
!if IS_opt_weighted=1, this file must be indicated
character(len=120) :: rapid_noweight_coupling_file
!if IS_opt_weighted=0, this file must be indicated
character(len=120) :: f_params_MC
!if IS_opt_routing=3: Muskingum-Cunge channel params
character(len=120) :: f_params_ov
!if IS_opt_ov=1: turn on overland time delay
character(len=120) :: f_Q_V_table
!if IS_opt_routing=5: use MC_HAND and read HAND geometry
...
!---lpr: MC paramters----
integer,dimension(:), allocatable :: comid
integer,dimension(:), allocatable :: tp  !'Connector' 'StreamRiver' etc
real,dimension(:), allocatable :: bw,ss,bs,ll,nchan,hinit,acat
real,dimension(:), allocatable :: llcat,ncat,slp,tlag
real,dimension(:,:), allocatable :: vh,qh,twh  !store Q-V based on HAND geometry
Vec :: MC_C1,MC_C2,MC_C3,MC_C4,MC_A
Vec :: MC_Km,MC_X,MC_Ck

!---lpr: parameters for overland flow
Vec :: ZV_Qlat_r  !lateral flow reaching river channel
Vec :: ZV_Qstore !overland flow storage
Vec :: ZV_Qstoreprev !overland flow storage

real :: speed1,speed2lb,speed2b,speed3b,speed3db !for temperory speeding up certain rivers

!*******************************************************************************
!Namelist
!*******************************************************************************
namelist /NL_namelist/                                                         &
                       speed1,speed2lb,speed2b,speed3b,speed3db,               &
                       BS_opt_Qinit,BS_opt_Qfinal,                             &
                       BS_opt_hum,BS_opt_for,BS_opt_dam,BS_opt_influence,      &
                       IS_opt_routing,IS_opt_run,IS_opt_phi,                   &
                       IS_opt_ov,beta,IS_opt_weighted,                         &
                       rapid_noweight_coupling_file,rapid_weighted_file,       &
                       f_params_MC,f_params_ov,f_Q_V_table,                    &
