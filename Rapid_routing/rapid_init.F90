...
use rapid_var, only :                                                          &
                   ...
                   comid,bw,ss,bs,ll,nchan,hinit,tp,acat,&  !lpr: MC parameters
                   MC_C1,MC_C2,MC_C3,MC_C4,MC_A, & !lpr: MC parameters
                   MC_Km,MC_X,MC_Ck, & !lpr: MC parameters
                   llcat,ncat,slp,tlag,IS_opt_ov, & !lpr: overland flow parameters
                   f_params_MC,f_params_ov,f_Q_V_table, & !lpr:input parameter files
                   IS_opt_weighted,rapid_weighted_file,&
                   rapid_noweight_coupling_file,  & !lpr:for consideration of weighting
                   vh,qh,twh !lpr: reading Q-V relation derived from HAND
...
integer :: iriv,nriv,itmp
integer, dimension(:,:),allocatable :: tmpid
integer, dimension(:,:),allocatable :: tmpno
...
if (rank==0 .and. IS_opt_routing==4)                       print '(a70)',      &
       'Routing with Muskingum-Cunge method (lpr, April 2017)                  '
if (rank==0 .and. IS_opt_routing==5)                       print '(a70)',      &
       'Routing with Muskingum-Cunge using HAND geometry (lpr, April 2017)     '
...
if (rank==0 .and. IS_opt_run==2)                           print '(a10,a60)',  &
       'Using    :',kfac_file 
if (rank==0 .and. IS_opt_ov==1)                            print '(a70)',      &
       'Routing with consideration for OVERLAND delay                          '
if (rank==0 .and. IS_opt_ov==0)                            print '(a70)',      &
       'Routing WITHOUT consideration for OVERLAND delay                       '
if (IS_opt_weighted==1)                      print '(a70)',      &
       'Routing using area-weighted average coupling interface                 '
if (IS_opt_weighted==0)                      print '(a70)',      &
       'Routing using no weight: centroid-based method                         '
...
!-------------------------------------------------------------------------------
!---lpr: modified--
if (IS_opt_routing==1 .or. IS_opt_routing==2) then
...
!************ROUTING OPTION 4: Muskingum-Cunge ********************************
...
allocate(comid(int(IS_riv_bas)))
allocate(bw(int(IS_riv_bas)))
allocate(ss(int(IS_riv_bas)))
allocate(bs(int(IS_riv_bas)))
allocate(ll(int(IS_riv_bas)))
allocate(nchan(int(IS_riv_bas)))
allocate(hinit(int(IS_riv_bas)))
allocate(tp(int(IS_riv_bas)))
allocate(acat(int(IS_riv_bas)))
!---catchment parameter table---
allocate(llcat(int(IS_riv_bas)))
allocate(ncat(int(IS_riv_bas)))
allocate(slp(int(IS_riv_bas)))
allocate(tlag(int(IS_riv_bas)))
!----read channel paramter table: following stream order--
nriv = INT(IS_riv_bas)
open(40,file=f_params_MC,status='old')
do iriv=1,nriv
        read(40,*) comid(iriv),bw(iriv),ss(iriv),bs(iriv),ll(iriv),&
                nchan(iriv),hinit(iriv) !,tp(iriv),acat(iriv)
        !print *,comid(iriv)
end do
print *,'	--- read MC channel parameter successful --- '  

if(IS_opt_ov==1) then 
open(50,file=f_params_ov,status='old')
do iriv=1,nriv
        read(50,*) comid(iriv),llcat(iriv),ncat(iriv),slp(iriv),tlag(iriv)
end do
print *,'    --- read OV catchment parameter successful --- ' 
end if
!---lpr: end Muskingum-Cunge initialization
end if
!************ROUTING OPTION 4: Muskingum-Cunge END********************************


!************ROUTING OPTION 5: Muskingum-Cunge with HAND *******************
if(IS_opt_routing==5) then
print *,'	*** RAPID routing using Muskingum-Cunge Method ***'
print *,'	*** Using Q-V relationship derived from HAND geometry'
print *,'	*** Variable flow velocity as a function of Q ****'
allocate(comid(int(IS_riv_bas)))
allocate(bw(int(IS_riv_bas)))
allocate(ss(int(IS_riv_bas)))
...
print *,'--- Note: HAND derived 83 Q-V relations---'
allocate(vh(int(IS_riv_bas),83))
allocate(qh(int(IS_riv_bas),83))
allocate(twh(int(IS_riv_bas),83))
allocate(tmpid(int(IS_riv_bas),83))
allocate(tmpno(int(IS_riv_bas),83))
...
open(66,file=f_Q_V_table,status='old')
do iriv=1,nriv
do itmp=1,83
        read(66,*) tmpid(iriv,itmp),tmpno(iriv,itmp),&
                vh(iriv,itmp),qh(iriv,itmp),twh(iriv,itmp)
        !qh(iriv,itmp)=qh(iriv,itmp)*0.3048*0.3048*0.3048 !Note cfs to cms: HAND input cfs
        !vh(iriv,itmp)=vh(iriv,itmp)*0.3048
        !twh(iriv,itmp)=twh(iriv,itmp)*0.3048
end do
end do
print *,'   *** read HAND Q-V table successful *** '
print *,'       test reach 1: ',tmpid(1,2),tmpno(1,2),vh(1,2),&
        qh(1,2),twh(1,2)

end if
!************ROUTING OPTION 5: Muskingum-Cunge with HAND END *******************
...
!----lpr: end Muskingum-Cunge using HAND geometry-------------
