...
use rapid_var, only : namelist_file,                                  &
                      ...
                      ierr,stage,rank,                                &
                      ZS_TauR,IS_opt_weighted,rapid_noweight_coupling_file,rapid_weighted_file, &
                      ZV_Qstore,ZV_Qstoreprev,ZS_one !vars for overland flow
...
!---variable for coupling file -----------
integer, dimension(:),allocatable :: npoints   !LPR record number of weights
real, dimension(:,:),allocatable :: ZV_areakm_wgt !at most 200 areas
real, dimension(:,:),allocatable :: weights !at most 200 wgts for all river reaches
integer, dimension(:,:),allocatable :: IV_i_index_wgt
integer, dimension(:,:),allocatable :: IV_j_index_wgt

integer :: tmp ! for temporary looping
integer :: tmpnpoints
...
!    use rapid_main , only : rapid_main_exe
    ...
    runoff = runoff !/ZS_TauR*3600 !*120  !if LSM=3hrly, original runoff is in 
    !note bug fix: unit must be in total mm in a time step (lpr May 2017)
...
400     format('0',i4)
      ...
      print *,'--- RAPID output = ',trim(Qout_nc_file),' ---'
    !end if
...
!-----------RAPID initialization call----------------------------------------------
  subroutine rapid_ini(NTIME)
    ...
    call VecSet(ZV_Qstoreprev,0*ZS_one,ierr)                 !Qstoreprev=0
    call VecSet(ZV_Qstore,0*ZS_one,ierr)                     !Qstore=0
...
!--------------RAPID coupler: gridded runoff to vector runoff-----------------------  
  subroutine rapid_runoff_to_inflow(ZM_runoff,ZV_Vlat,cnt_rapid_run)
    ...
    Vec, intent(out) :: ZV_Vlat   ! total volume Qlat in a LSM time step: m3
    integer :: cnt_rapid_run
    ...
    if (rank==0) then
      ...
      if (cnt_rapid_run==1) then
        !---------First time: READ coupling files------------------------
        if(IS_opt_weighted==0) then
        !----------OPTION 1: Catchment centroid-based coupling-----------
        print *,' --- reading coupling: no weight (centroid-based file) ---'
        !print *,'         file from ',rapid_noweight_coupling_file
        allocate(IV_i_index(IS_riv_tot))
        allocate(IV_j_index(IS_riv_tot))
        allocate(ZV_areakm(IS_riv_tot))
        open(88,file=rapid_noweight_coupling_file,status='old')
        do JS_riv_tot=1,IS_riv_tot
          read(88,*) IV_riv_bas_id(JS_riv_tot),ZV_areakm(JS_riv_tot), &
                   IV_i_index(JS_riv_tot),IV_j_index(JS_riv_tot)
        end do
        close(88)
        !print *,' LPR CHECK river 300 ',IV_riv_bas_id(300),ZV_areakm(300), &
        !        IV_i_index(300),IV_j_index(300)        
        print *,' --- RAPID: read coupling file [centroid] successfully ---'
        !---------END OPTION 1----------------------------------

        else if(IS_opt_weighted==1) then 
        !---------OPTION 2: Area-weighted coupling----------------------
        print *,' --- reading coupling: area-weight (weighting file needed) ---'
        !print *,'         file from ',rapid_weighted_file
        allocate(IV_i_index_wgt(IS_riv_tot,200))  ! max wgt allowed: 200
        allocate(IV_j_index_wgt(IS_riv_tot,200))
        allocate(ZV_areakm_wgt(IS_riv_tot,200))
        allocate(npoints(IS_riv_tot))
        allocate(weights(IS_riv_tot,200))
        open(88,file=rapid_weighted_file,status='old')
        do JS_riv_tot=1,IS_riv_tot
          weight_count=1
          read(88,*) IV_riv_bas_id(JS_riv_tot),ZV_areakm_wgt(JS_riv_tot,weight_count), &
                   IV_i_index_wgt(JS_riv_tot,weight_count),IV_j_index_wgt(JS_riv_tot,weight_count),&
                   npoints(JS_riv_tot),weights(JS_riv_tot,weight_count)
          if(npoints(JS_riv_tot) > 1) then
                do tmp=1,npoints(JS_riv_tot)-1
                        weight_count=weight_count+1
                        read(88,*) IV_riv_bas_id(JS_riv_tot),&
                                ZV_areakm_wgt(JS_riv_tot,weight_count),&
                                IV_i_index_wgt(JS_riv_tot,weight_count),&
                                IV_j_index_wgt(JS_riv_tot,weight_count),&
                                tmpnpoints,weights(JS_riv_tot,weight_count)
                        if(weight_count>200) then
                                stop '    Weight exceeds 200: max number of intersecting cells'
                        end if
                end do
           end if
           !if(JS_riv_tot==IS_riv_tot) then
           !     print *,'       --- LPR CHECK: last reach in weight table ---'
           !     print *,'       ID=',IV_riv_bas_id(JS_riv_tot),ZV_areakm_wgt(JS_riv_tot,weight_count), &
           !             IV_i_index_wgt(JS_riv_tot,weight_count),IV_j_index_wgt(JS_riv_tot,weight_count),&
           !             npoints(JS_riv_tot),weights(JS_riv_tot,weight_count)
           !end if
        end do
        close(88)
        print *,' --- RAPID: read coupling file [weighted] successfully ---'
        print *,'   if segmentation fault, check river reach number in wgt table'
        !--------END OPTION 2-----------------------------------
        else !if IS_opt_weighted>1, ERROR
        stop '  !!! ******ERROR with namelist IS_opt_weighted file **** !!!'
        end if
      ...
      if(IS_opt_weighted==0) then
      !--------OPTION 1: Catchment centroid-based coupling-----------
      !print *,'         --- actual coupling: no weight (centroid-based) ---'
      do JS_riv_tot=1,IS_riv_tot
      ...
      !---------OPTION 2: Area-weighted coupling----------------------
      !print *,'         --- actual coupling: weighted (ArcGIS generated weighting file) ---'
      do JS_riv_tot=1,IS_riv_tot
        weight_count=1
        if(npoints(JS_riv_tot) > 1) then
                do tmp=1,npoints(JS_riv_tot)
                        JS_lon=IV_i_index_wgt(JS_riv_tot,weight_count)
                        JS_lat=IV_j_index_wgt(JS_riv_tot,weight_count)
                        ZV_read_riv_tot(JS_riv_tot)=&
                                ZV_read_riv_tot(JS_riv_tot)&
                                +ZM_runoff(JS_lon,JS_lat) &
                                *ZV_areakm_wgt(JS_riv_tot,weight_count)*1000 !&
                                !*weights(JS_riv_tot,weight_count)
                        !if(IV_riv_bas_id(JS_riv_tot)==1508279) then
                        !        print *,"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
                        !        print *,"id=",IV_riv_bas_id(JS_riv_tot)
                        !        print*,"nx=",IV_i_index_wgt(JS_riv_tot,weight_count)
                        !        print*,"ny=",IV_j_index_wgt(JS_riv_tot,weight_count)
                        !        print*,"area=",ZV_areakm_wgt(JS_riv_tot,weight_count)
                        !        print*,"weight=",weights(JS_riv_tot,weight_count)
                        !        print *,"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
                        !end if
                        weight_count=weight_count+1
                end do
      ...
      !check output
      !if(JS_riv_tot .eq. IS_riv_tot) then
      !  print *,'       --- LPR CHECK last river--- m3_riv = ',ZV_read_riv_tot(JS_riv_tot)
      !end if
      else !if IS_opt_weighted>1, ERROR
      stop '  !!! ***** ERROR with namelist IS_opt_weighted file ****!!!'
      end if
