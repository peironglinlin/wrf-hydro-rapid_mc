...
!---lpr: added for Muskingum-Cunge routing---
use rapid_routing_MC,only: routing_params_MC
use rapid_routing_MC_HAND, only: routing_params_MC_hand !reading new channel geometry
...
use rapid_var, only :                                                          &
                   ...
                   IS_opt_routing,comid,bw,ss,bs,ll,nchan,hinit,tp,acat,& !lpr added
                   MC_C1,MC_C2,MC_C3,MC_C4,MC_A,MC_Km,MC_X,&  !lpr added
                   ksp,ZM_A,ZV_k,ZV_x, & !lpr added
                   llcat,ncat,slp,tlag, &  !lpr: overland flow parameters
                   vh,qh,twh  !lpr: added for Q-V table from HAND
...
!!! lpr note: ZV_Vlat -> total runoff accumulated over ZS_TauR (the LSM time step)
call VecCopy(ZV_Vlat,ZV_Qlat,ierr)            !Qlat=Vlat
!!! lpr note: ZV_Qlat should be in the unit of m3/s
...
if(IS_opt_routing.eq.1.or.IS_opt_routing.eq.2.or.IS_opt_routing.eq.3) then
call rapid_routing(ZV_C1,ZV_C2,ZV_C3,ZV_Qext,                                  &
                   ZV_QoutinitR,ZV_VinitR,                                     &
                   ZV_QoutR,ZV_QoutbarR,ZV_VR,ZV_VbarR)
end if
!---lpr: Muskingum-Cunge---
if (IS_opt_routing.eq.4) then
call routing_params_MC(comid,bw,ss,bs,ll,nchan,hinit,tp,acat,&
                llcat,ncat,slp,tlag,&
...
!---lpr: Muskingum-Cunge with Q-V relation derived from HAND river geometry---
if (IS_opt_routing.eq.5) then
call routing_params_MC_hand(comid,bw,ss,bs,ll,nchan,hinit,tp,acat,&
                llcat,ncat,slp,tlag,&
                vh,qh,twh, &
                ZV_Qext,ZV_QoutinitR, &
                ZV_C1,ZV_C2,ZV_C3,MC_C4,ZM_A,ZV_k,ZV_x,&
